...
Looper.prototype.create_mix = function ( song ) {
  ...
  */
  ...
  var before = this.position, beats = [];
  for(m; m < song[0].length; m++){
    for(b = 0; b < song[0][m].length; b++){
      var loc = (m>0) ? (m*song[0][m].length) + b : b;
...
Looper.prototype.step = function (steps) {
  ...
  //
  function next() {
    if (this.position > this.theMix[0])
    
    if (!self.paused && (Date.now() - start) > 60000/ (self.bpm * self.measures)) {
